...
(() => {
  ...
  const CALC_PAGE_VERSION = "2026-06-18-drawdown-disable-logics-v6";
  ...
  function disableAllLogicsForDrawdown(meta) {
    const pd = ensurePortfolioDrawdownState();
    pd.disabled = true;
    ...
    for (const key of selectedLogicIds()) disableLogicForDrawdown(key, meta, { skipSave: true });
    syncLogicChipDrawdownState();
    saveConfig();
  }

  function enableAllLogicsAfterDrawdown(meta) {
    const pd = ensurePortfolioDrawdownState();
    pd.disabled = false;
    pd.peakEquity = meta?.equity ?? pd.peakEquity;
    pd.resumeAt = null;
    pd.pausedAt = null;
    for (const key of selectedLogicIds()) enableLogicAfterDrawdown(key, meta, { skipSave: true });
    syncLogicChipDrawdownState();
    saveConfig();
  }
    ...
    for (const ev of events) {
      if (ev.kind === "pause") {
        if (cfg.perLogic && ev.logicKey) {
          disableLogicForDrawdown(ev.logicKey, ev);
        } else if (!cfg.perLogic) {
          disableAllLogicsForDrawdown(ev);
        }
      } else if (ev.kind === "resume") {
        if (cfg.perLogic && ev.logicKey) {
          enableLogicAfterDrawdown(ev.logicKey, ev);
        } else if (!cfg.perLogic) {
          enableAllLogicsAfterDrawdown(ev);
        }
      }
    }
    syncLogicChipDrawdownState();
