...
(function (root) {
  ...
  const HELP = {
    ...
    PIK: [
      "PIK — взятие пиков корзины (mean-reversion). Работает в расчёте FINRESP, live и песочнице.",
      "",
      "Корзина TOT = среднее close всех выбранных бумаг (как график «Средневзвешенная цена»).",
      "",
      "Формулы (на баре t, N бумаг, цена i: p_i,t):",
      "  A_t = (Σ p_i,t) / N",
      "  TotStoch — стохастик по ряду A_t.",
      "  Вход long (откуп провала): TotK≤12, локально StochK≤15, MOM_5<0.",
      "  Выход long: TotK≥42 и StochK≥42 (возврат к середине).",
      "  Вход short (продажа всплеска): TotK≥88, StochK≥85, MOM_5>0.",
      "  Выход short: TotK≤55 и StochK≤55.",
      "",
      "Рекомендации: TF расчёта 10m; ≥15 бумаг в списке; основная сессия 10:00–18:40 МСК.",
      "Не включать в стек с PIKH. SL[@SL] TP[@TP] — из параметров (@SL=1.5…2, @TP=4…6 для игл)."
    ].join("\n"),

    PIKH: [
      "PIKH — горизонтальный режим корзины (боковик). Альтернатива PIK, не смешивать в одном расчёте.",
      "",
      "Условие «горизонт»: 32 ≤ TotK ≤ 68, цена внутри полос Bollinger(30), локальный Stoch на экстремуме.",
      "Long: откуп у нижней части диапазона; Short: продажа у верхней.",
      "",
      "Формула фильтра боковика: |A_t − median(A)| / median(A) < ε на окне (в логике — TotStoch в середине).",
      "PIK ловит вертикальные иглы (|ΔA| большой); PIKH — мелкие колебания без охоты за 1‑минутными артефактами."
    ].join("\n")
  };
