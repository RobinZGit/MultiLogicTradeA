...
${payload.issues?.lastError ? `<p><strong>Замечание:</strong> ${esc(payload.issues.lastError)}</p>` : ""}
  ...
  function bindLiveSessionClearUi() {
    ...
    btn.addEventListener("click", () => {
      ...
      const extra = sandbox
        ? " Позиции и журнал песочницы будут сброшены к депозиту. Выбранные логики останутся; сбросятся паузы просадки и кэш equity."
        : " Позиции на бирже не затрагиваются — очищается журнал в браузере. Выбранные логики останутся; сбросятся паузы просадки и кэш equity.";
