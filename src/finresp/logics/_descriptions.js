...
(function (root) {
  ...
  const HELP = {
    ...
    FTS: [
      "Фьючерсы: CtgStoch + Stoch, лонг, контртренд 20↔80.",
      "",
      "CtgStoch — стохастик по контанго (фьючерс − базовый актив).",
      "Op(Long(...)) — CtgStoch(K<=20) AND Stoch(K<=20): обе линии в перепроданности.",
      "Cl(Long(...)) — обе K>=80: перекупленность, выход.",
      "SL[@SL] TP[@TP] — из параметров."
    ].join("\n"),

    FTT: [
      "Фьючерсы: CtgStoch + Stoch, лонг, тренд 80↔20.",
      "",
      "Op(Long(...)) — K>=80 на обоих стохастиках (импульс вверх).",
      "Cl(Long(...)) — K<=20 (импульс выдохся).",
      "SL[@SL] TP[@TP] — из параметров."
    ].join("\n"),

    FTS_S: [
      "Фьючерсы: CtgStoch + Stoch, шорт, контртренд 80↔20.",
      "",
      "Op(Short(...)) — K>=80 (перегретость → шорт).",
      "Cl(Short(...)) — K<=20 (остывание → закрытие).",
      "SL[@SL] TP[@TP] — из параметров."
    ].join("\n"),

    FTT_S: [
      "Фьючерсы: CtgStoch + Stoch, шорт, тренд 20↔80.",
      "",
      "Op(Short(...)) — K<=20 (слабость → шорт по тренду вниз).",
      "Cl(Short(...)) — K>=80.",
      "SL[@SL] TP[@TP] — из параметров."
    ].join("\n"),
